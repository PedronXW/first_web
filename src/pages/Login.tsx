                ...
                <form onSubmit={handleSubmit(log)} autoComplete="off" className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-2">
                        <PasswordInput register={register} focus={setFocus} resetError={()=>{clearErrors('password')}}/>
                            {errors.password ? 
                                <h3 className="h-5 text-xs text-red-500 pl-2">{errors!.password!.message?.toString()}</h3> : 
                                <h3 className="h-5"> </h3>}
                    </div>
                    <button type="submit" className="w-full p-2 cursor-pointer flex items-center bg-primary_color border-primary_color rounded-md text-secundary_color justify-center">Login</button>
                    <h3 className="h-8 w-auto p-2 flex cursor-pointer text-xs self-center"><Link to={'/reset-password'}>Forgeted your Password?</Link></h3>
                </form>
            </div>
        </div>
    )
