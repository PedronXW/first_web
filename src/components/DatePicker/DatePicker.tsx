...
const DatePicker = ({ onStartSelected, onEndSelected }: DatePickerProps) => {
  ...
  const lastDateThisMonth = new Date(year, month + 1, 0)
  ...
  for (
    let j = lastDateThisMonth.getDate() + 1;
    new Date(year, month, j).getDay() !== 0 || days.length !== 42;
    j++
  ) {
  ...
  function setStyle(day: Date) {
    if (
      endSelected?.getTime() === day.getTime() &&
      startSelected?.getTime() === day.getTime() &&
      endSelected?.getTime() === startSelected?.getTime()
    ) {
    ...
    if (
      day.getTime() > startSelected?.getTime()! &&
      day.getTime() < endSelected?.getTime()!
    ) {
    ...
    if (
      day.getTime() <= lastDateThisMonth?.getTime()! &&
      day.getTime() > new Date(year, month, 0)?.getTime()!
    ) {
    ...
    <div className="h-min w-full flex flex-col gap-4">
      <div className="flex justify-between">
        <button
          onClick={voltarMes}
          type="button"
          className="text-primary_color text-2xl font-bold"
        >
          <CaretLeft size={20} />
        </button>
        <p className="text-primary_color text-md font-bold">
          {month + 1}/{year}
        </p>
        <button
          onClick={passarMes}
