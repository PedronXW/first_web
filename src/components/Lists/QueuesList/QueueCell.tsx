...
const QueueCell = ({ queue }: QueueCellInterface) => {
    ...
    return (
        <li onClick={()=>{navigate("cdscnsaocsadcsakl")}} title={queue.name ? queue.name : "arroz"} className={`h-36 flex-col w-full justify-end items-end flex`}>
            ...
            <div className="h-5/6 w-full rounded-lg self-end bg-secundary_color drop-shadow-3xl cursor-pointer align-bottom -mt-4">
                ...
                <div className="h-2/4 w-full bg-secundary_color rounded-b-lg flex items-center justify-between">
                    ...
                    <strong className="text-primary_color font-medium h-full flex justify-end items-center pr-7 w-min">{queue.digit}</strong>
                </div>
            </div>
        </li>
    )
