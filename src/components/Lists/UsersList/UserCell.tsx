...
const UserCell = ({ user }: UserCellInterface) => {
  ...
  return (
    <div
      className="min-h-[144px] xl:max-w-[400px] flex flex-col w-full justify-end items-end cursor-pointer"
      onClick={() => {
        navigate('xcdqhbdskhascbhsa')
      }}
    >
      ...
      <div className="h-5/6 w-full rounded-lg self-end drop-shadow-3xl align-bottom -mt-7">
        ...
        <div className="h-2/4 w-full bg-secundary_color rounded-b-lg flex items-center justify-center px-5">
          <strong className="text-primary_color font-medium h-full flex justify-start items-center w-1/2 whitespace-nowrap overflow-ellipsis">
            Pedro de Almeida
          </strong>
          <strong className="text-primary_color font-medium h-full flex justify-end items-center w-1/2">
            (35)99124-4060
          </strong>
        </div>
      </div>
